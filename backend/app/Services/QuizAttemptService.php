...
class QuizAttemptService
{
    ...
    public function start(User $user, Quiz $quiz): QuizAttempt
    {
        ...
        if (! $enrollment) throw ValidationException::withMessages(['enrollment' => 'An active enrollment is required.']);
        ...
        return DB::transaction(function () use ($user, $quiz, $enrollment): QuizAttempt {
            ...
            if ($active) $active->update(['status' => 'expired', 'completed_at' => now()]);
    ...
    public function submit(QuizAttempt $attempt, array $submittedAnswers): QuizAttempt
    {
        return DB::transaction(function () use ($attempt, $submittedAnswers): QuizAttempt {
            ...
            if ($attempt->status !== 'in_progress') throw ValidationException::withMessages(['attempt' => 'This attempt has already been finalized.']);
            ...
            foreach ($quiz->questions as $question) {
                ...
                if ($answerId && ! $selected) throw ValidationException::withMessages(['answers' => 'An answer does not belong to its question.']);
