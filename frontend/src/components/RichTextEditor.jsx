...
export default function RichTextEditor({ value, onChange }) {
  ...
  const commands = [['bold', t('editor.bold')], ['italic', t('editor.italic')], ['insertUnorderedList', t('editor.list')]];
  ...
  return (
    <div className="rounded-xl border border-slate-300 overflow-hidden bg-white">
      ...
      <div ref={editor} contentEditable role="textbox" aria-label={t('editor.area')} onInput={event => onChange(event.currentTarget.innerHTML)} className="min-h-48 p-4 outline-none prose max-w-none" />
    </div>
  );
}
